...
#         "- Logic correctness: 0 to 30 points\n"
#         "- Time complexity: 0 to 10 points\n"
#         "- Space complexity:  0 to 10 points\n"
#         "- Syntax correctness: 0 to  10 points\n\n"
#         "- Low level Design solution: 0 to 20 points\n\n"
#         "- Efficient algorithm used :  0 to 10 points\n\n"
#         "- Scalablity of the solution:  0 to 10 points\n\n"
...
def evaluate_answer(question, answer):
    prompt = (
        "You are an AI interviewer evaluating a candidate's solution to a coding problem.\n\n"
        "You MUST evaluate with maximum strictness. If the answer does not meaningfully solve the problem, "
        "the score MUST be 0 for ALL categories.\n\n"

        "STRICT SCORING RULES:\n"
        "- If the logic does not solve the problem → Logic = 0\n"
        "- If the answer ignores input/output requirements → Logic = 0\n"
        "- If solution is hardcoded or returns a constant → Logic = 0\n"
        "- If no algorithm is implemented → Time = 0, Space = 0\n"
        "- If the answer is short and lacks reasoning → LowLevelDesign = 0\n"
        "- If no algorithmic thinking is shown → EfficientAlgorithm = 0\n"
        "- If solution cannot scale → Scalability = 0\n"
        "- Syntax > 0 ONLY if the code is syntactically valid\n"
        "- If the answer is irrelevant, empty, or placeholder → TotalScore MUST be 0\n\n"

        "SCORING BREAKDOWN (TOTAL = 100):\n"
        "- Logic correctness: 0–30\n"
        "- Time complexity: 0–10\n"
        "- Space complexity: 0–10\n"
        "- Syntax correctness: 0–10\n"
        "- Low-level design quality: 0–20\n"
        "- Efficient algorithm usage: 0–10\n"
        "- Scalability: 0–10\n\n"

        "For each category, provide:\n"
        "- A short explanation\n"
        "- Actual complexity (O(n), O(1), etc.) where applicable\n\n"
